...
export const help = async (arg, pkg, log) => {
    await import("colors");
...
export const parse = async function (e) {
    await import("colors");
    ...
    return new Promise((ok, no) => {
        ...
        let optKeywords = [];
        for (var o in avKeywords[cs.APP_DEFFUNC].options)
            optKeywords = optKeywords.concat(cs.APP_OPT[avKeywords[cs.APP_DEFFUNC].options[o]].flags);
        ...
        let processArgs = (v, i, a) => {
            if (i == 0) {
                ...
                else if (optKeywords.includes(v)) {
                ...
                if (typeof analyzed.fn.options != "undefined")
                    for (var i in analyzed.fn.options) {
                        if (cs.APP_OPT[analyzed.fn.options[i]].required)
                            analyzed._reqOptList.push(analyzed.fn.options[i]);
                        for (var j in cs.APP_OPT[analyzed.fn.options[i]].flags)
                            analyzed.optionSet[
                                cs.APP_OPT[analyzed.fn.options[i]].flags[j]
        ...
        if (typeof optDataComes === "string")
            return no(e.make(0x5, `Option value for '${optDataComes}' option is missing.`));
        ...
        if (analyzed._reqOptList.length > 0) {
            ...
            for (var opt in analyzed._reqOptList) {
                let tempObj = {};
                tempObj[
                    cs.APP_OPT[analyzed._reqOptList[opt]].flags.join(" or ")
                ] = cs.APP_OPT[analyzed._reqOptList[opt]].description;

                arrForHelp.push(tempObj);
            }

            return no(e.make(0x4, "Required option doesn't set.", arrForHelp));
        } else {
